...
import { getPendingNotifications, markNotificationSent } from "@/lib/notification-scheduler"
        ...
        try {
          ...
          if (eventData) {
            ...
            if (success) {
              ...
              console.log(`[v0] Sent notification for event: ${eventData.title}`)
            } else {
              console.error(`[v0] Failed to send notification for event: ${eventData.title}`)
            }
          }
        } catch (error) {
          console.error("[v0] Error processing individual notification:", error)
        }
      }
    } catch (error) {
