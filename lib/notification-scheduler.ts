...
function calculateNotificationTimes(eventDate: string, settings: NotificationSettings): ScheduledNotification[] {
...
export async function scheduleNotifications(
  ...
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/notification_queue`, {
      method: "POST",
      headers: {
    ...
    console.log("[v0] Scheduled", notificationsToCreate.length, "notifications for event", eventId)
    return true
  } catch (error) {
...
export async function clearEventNotifications(eventId: string): Promise<boolean> {
  try {
    const authResult = await getUser()
    if (!authResult || !authResult.user || !authResult.access_token) return false
...
export async function getPendingNotifications(): Promise<ScheduledNotification[]> {
  try {
    ...
    if (!response.ok) {
      console.error("Error fetching pending notifications:", response.statusText)
      return []
    }
